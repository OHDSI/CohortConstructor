...
To create a cohort from a concept set and include records outside of the observation period, we can set `useRecordsOutOfObservation = TRUE`. If we also want to search for the given concepts in the source concept_id fields, rather than only the standard concept_id fields, we can set `useSourceFields = TRUE`.
...
It is also possible to include records outside of observation by setting the `useRecordsOutOfObservation` argument to TRUE.
