test_that("subsetCohort works", {
  ...
  # same name
  cohort <- cdm$cohort1
  expect_warning(cdm$cohort1 <- subsetCohorts(cdm$cohort1, c(3,4,5)))
...
test_that("codelist works", {
  ...
  cdm <- omock::mockCdmFromTables(tables = list("cohort1" = cohort_1)) |>
    ...
    omopgenerics::insertTable(name = "drug_exposure", table = dplyr::tibble(
      ...
      "person_id" = as.integer(c(1, 1, 1, 1, 2, 2, 3, 1, 1, 1, 1, 4, 4, 1, 2, 3, 4)),
      "drug_concept_id" = as.integer(c(1, 1, 1, 2, 1, 1, 2, 1, 1, 1, 1, 1, 2, 3, 3, 3, 3)),
  ...
  # same name
  cohort <- cdm$cohort1
  expect_warning(cdm$cohort1 <- subsetCohorts(cdm$cohort1, 2))
...
test_that("Expected behaviour", {
  ...
  expect_warning(expect_warning(cdm$cohort2 <- subsetCohorts(cdm$cohort1, "1", name = "cohort2")))
  expect_error(cdm$cohort2 <- subsetCohorts(cdm$cohort1, 2, name = "cohort3"))
  expect_warning(expect_warning(cdm$cohort2 <- subsetCohorts(cdm$cohort1, 10, name = "cohort2")))
...
test_that("test indexes - postgres", {
  ...
  if (dbToTest == "postgres CDMConnector") {
    ...
    expect_warning(cdm$my_cohort <- subsetCohorts(cdm$my_cohort, cohortId = 1))
    expect_true(
