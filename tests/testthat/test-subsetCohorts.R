...
test_that("Expected behaviour", {
  ...
  expect_warning(cdm$cohort2 <- subsetCohorts(cdm$cohort1, 10, "cohort2"))
  expect_true(dplyr::tally(cdm$cohort2) |> dplyr::pull("n") == 0)
  expect_true(nrow(attrition(cdm$cohort2)) == 0)
  expect_true(nrow(settings(cdm$cohort2)) == 0)
  expect_true(nrow(cohortCount(cdm$cohort2)) == 0)
